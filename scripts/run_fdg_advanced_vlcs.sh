#!/usr/bin/env bash
#
# Run advanced FedDG baselines on VLCS (ResNet-50 backbone, the default) under
# the non-IID, large-client regime that matches run_fdg_advanced_minidomainnet.sh:
#
#     num_clients = 100
#     iid         = 0.0
#     fraction    = 0.2
...
#     num_workers = 4
#
# Default L2DO baked into resources/vlcs_v1.0/metadata.csv:
#     val=SUN, test=PASCAL. Train pool = {CALTECH, LABELME}.
# Train pool has ~2,563 rows -> ~25 rows per client at num_clients=100.
# Note: VLCS is small. With batch_size=32 each client holds < 1 batch; metrics
# may be noisier than on the larger datasets. Consider NUM_CLIENTS=10 or
# BATCH_SIZE=16 overrides for more stable local steps if you do not need
# the exact same regime as the other sweeps.
#
...
#
# Usage:
#   bash scripts/run_fdg_advanced_vlcs.sh                  # all 7, wandb on
#   ROUNDS=30 bash scripts/run_fdg_advanced_vlcs.sh        # override num_rounds for all
#   METHODS="FedProx FedSR" bash scripts/run_fdg_advanced_vlcs.sh   # subset
#   NO_WANDB=1 bash scripts/run_fdg_advanced_vlcs.sh       # disable wandb
#   WANDB_GROUP=my_sweep bash scripts/run_fdg_advanced_vlcs.sh
...
WANDB_GROUP="${WANDB_GROUP:-vlcs_n100_iid0_frac20}"
WANDB_PROJECT_NAME="${WANDB_PROJECT_NAME:-FedDG_Benchmark_vlcs}"

LOG_DIR="logs/fdg_advanced_vlcs"
...
c["data_path"] = "resources/vlcs_v1.0/"
