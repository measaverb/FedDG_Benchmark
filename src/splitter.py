...
class LeaveOneDomainOutSplitter(DatasetSplitter):
    ...
    def split(self, dataset, domain_field, transform=None):
        ...
        tar_idx = np.where(dataset.metadata_array[:, domain_field] == self.test_domain)[
            0
        ]
        rem_idx = np.where(dataset.metadata_array[:, domain_field] != self.test_domain)[
            0
        ]
        if isinstance(dataset, WILDSSubset):
            return WILDSSubset(
                dataset.dataset, indices[tar_idx].tolist(), transform=transform
            ), WILDSSubset(
                dataset.dataset, indices[rem_idx].tolist(), transform=transform
            )
        elif isinstance(dataset, WILDSDataset):
            return WILDSSubset(
                dataset, tar_idx.tolist(), transform=transform
            ), WILDSSubset(dataset, rem_idx.tolist(), transform=transform)
        else:
...
class RandomSplitter(DatasetSplitter):
    ...
    def split(self, dataset, transform=None):
        ...
        return WILDSSubset(
            ori_dataset, perm_indices[0:pos].tolist(), transform=transform
        ), WILDSSubset(ori_dataset, perm_indices[pos:].tolist(), transform=transform)
...
class NonIIDSplitter:
    ...
    def split(self, dataset, domain_field, transform=None):
        ...
        num_examples_per_domain = np.bincount(
            dataset.metadata_array[:, domain_field]
        )  # compute number of examples per domain.
        num_domains = len(
            num_examples_per_domain
        )  # number of domains. Notice that we use bincount, that means the metaarray should be integer at least.
        non_empty_num_domains = sum(num_examples_per_domain > 0)
        if non_empty_num_domains <= self.num_shards:
            main_shards_per_domain = (num_examples_per_domain > 0).astype(
                "int"
            )  # initialize the data structure to store each domain‘s main shards

            while (
                np.sum(main_shards_per_domain) < self.num_shards
            ):  # start to distribute domain to
                ratio = np.divide(
            ...
            main_domain_ratio_per_shard = np.array(
                [1 / u if u != 0 else 0 for u in main_shards_per_domain]
            )  # len = num_domains
            ...
            diff = np.rint(
                np.sum(np_examples_per_shards - np_int_examples_per_shards, axis=0)
            ).astype(int)
            diff_mask = np.zeros((self.num_shards, num_domains))
            for col in range(num_domains):
                diff_mask[0 : diff[col], col] = 1
                final_examples_per_shards = (
                    np.rint(np_int_examples_per_shards + diff_mask)
                ).astype(int)
            ...
            float_final_examples_per_shards = (
                num_examples_per_shards_1 * self.iid
                + num_examples_per_shards_0 * (1 - self.iid)
            )
            # could be float need to change to int.
            final_examples_per_shards = np.floor(
                float_final_examples_per_shards
            ).astype("int")
            diff_array = (
                num_examples_per_domain - np.sum(final_examples_per_shards, axis=0)
            ).astype("int")
        ...
        for shard in range(self.num_shards):
            ...
            for j, _ in enumerate(final_examples_per_shards[shard]):
                ...
                if offset > 0:
                    shard_indices += (
                        indices_per_domain[j][pointer[j] : pointer[j] + offset]
                    ).tolist()
                    pointer[j] += offset
            if isinstance(dataset, WILDSSubset):
                dataset_per_shards.append(
                    WILDSSubset(dataset.dataset, shard_indices, transform=transform)
                )
            elif isinstance(dataset, WILDSDataset):
                dataset_per_shards.append(
                    WILDSSubset(dataset, shard_indices, transform=transform)
                )
            else:
                raise NotImplementedError
        ...
        for i, dt in enumerate(dataset_per_shards):
            assert np.array_equal(
                np.array(
                    np.bincount(
                        dt.metadata_array[:, domain_field], minlength=num_domains
                    )
                ),
                np.array(final_examples_per_shards[i]),
            )
