...
from .models import CNN, Classifier, DenseNet, DistilBertFeaturizer, ResNet, code_gpt_py
...
class ObjBundle(object):
    def __init__(self, dataset, probabilistic=False) -> None:
        ...
        self.grouper = CombinatorialGrouper(
            dataset=dataset, groupby_fields=self.groupby_fields
        )
...
class CivilComments(ObjBundle):
    def __init__(self, dataset, probabilistic=False) -> None:
        ...
        self.train_transform = self._train_transform
        self.test_transform = self._test_transform
        self.featurizer = DistilBertFeaturizer.from_pretrained(
